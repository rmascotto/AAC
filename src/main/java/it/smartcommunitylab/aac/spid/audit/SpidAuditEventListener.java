...
package it.smartcommunitylab.aac.spid.audit;
...
public class SpidAuditEventListener implements ApplicationListener<SpidRequestEvent>, ApplicationEventPublisherAware {
    ...
    private ProviderConfigRepository<SpidIdentityProviderConfig> registrationRepository;

    @Autowired
    public void setRegistrationRepository(ProviderConfigRepository<SpidIdentityProviderConfig> registrationRepository) {
    ...
    public void onApplicationEvent(SpidRequestEvent event) {
        ...
        SpidIdentityProviderConfig config = registrationRepository.findByProviderId(provider);
        if (config == null) {
